...
class CampaingViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, pk=None):
        try:
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                return Response(
                        {'data': 'campaing updated successufully.'},
                        status=status.HTTP_200_OK
                )
        except Campaing.DoesNotExist as err:
            return Response(
    ...
    def destroy(self, request, pk=None):
        try:
            ...
            return Response(
                    {'data': 'campaing deleted successfully.'},
                    status=status.HTTP_200_OK
            )
        except Campaing.DoesNotExist as err:
            return Response(
...
class CampaingPublic(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Campaing.objects.filter(
                status_campaing=self.campaing_public
        )
        return queryset
    ...
    def retrieve(self, request, slug_title):
        try:
            ...
            current_campaing = get_object_or_404(
                    queryset,
                    slug=slug_title
            )
            serializer = serializers.CampaingListSerializer(
                    current_campaing
            )
            return Response(
                    {'data': serializer.data},
